...
| Use stable UUIDs and sync custom cards before dependent SRS records | Sync revision | Two devices cannot collide or poison pull progress | DONE (client + tests); `custom_cards` remote table migration written, apply PENDING (blocked by production-DDL guard) |
